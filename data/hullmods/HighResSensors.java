...
//		stats.getSensorStrength().modifyFlat(id, bonus);
		if (stats.getVariant().getSMods().contains("hiressensors") || stats.getVariant().getSModdedBuiltIns().contains("hiressensors")) {
...
//		if (index == 2) return "" + ((Float) mag.get(HullSize.CRUISER)).intValue();
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		if (isForModSpec) {
			...
			return;
		} else if (ship.getVariant().getSMods().contains("hiressensors") || ship.getVariant().getSModdedBuiltIns().contains("hiressensors")) {
