	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getVariant() != null) {
			mod = Math.max(stats.getVariant().getHullSpec().getFuel() * MIN_FRACTION, mod);
                ...
                if (stats.getVariant().getSMods().contains("auxiliary_fuel_tanks") || stats.getVariant().getSModdedBuiltIns().contains("auxiliary_fuel_tanks")) {
                    mod *= 2;
                }
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
        ...
        public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
                     if (isForModSpec) {
                             ...
                             return;
                     } else if (ship.getVariant().getSMods().contains("auxiliary_fuel_tanks") || ship.getVariant().getSModdedBuiltIns().contains("auxiliary_fuel_tanks")) {
